...
- [ ] `good-match-adjacent` is now the only pair whose exact expectation is `good_match`, and `good_match` was already the thinnest observed band at 4 and 5 occurrences of 80. If it ever moves, that band loses its exact expectation and `ground-truth.test.ts` will say so, which is the guard working, but the set would want a second `good_match` pair before then.

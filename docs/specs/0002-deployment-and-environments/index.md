...
Whether Supabase's setting withholds privileges from `service_role` as well as from `anon` and `authenticated` is not confirmable from inside this repository, and the evidence available leans the other way: the installed `supabase` skill describes the setting as affecting `anon` and `authenticated` only. The grant is written anyway. It costs one line, it grants nothing the design did not already intend, and its absence is expensive to diagnose. Task 10 checks the real privilege on the real project rather than leaving the question open, because that check is what tells you how to read a denial in task 11.
...
- [ ] **AC-12 is blocked by a platform fact this spec did not check: branch protection is not available on a private repository on the free GitHub plan.** Found during the build on 2026-08-21, from GitHub's own answer: both the rulesets and the classic branch protection endpoints return 403, "upgrade to GitHub Pro or make this repository public". `ghalynho10/JobHunt` is private. So `main` cannot be protected as written. Three ways out, and the choice is not the build's to make: make the repository public (free, and this is a portfolio project meant to be public, but it exposes the whole history, so it needs a check that no secret was ever committed), pay for GitHub Pro (against the free constraint this whole feature was designed under), or accept AC-12 unmet for now and rely on the discipline of opening a pull request. Task 17 is not done and the guardrails milestone cannot close until this is decided.
...
- [ ] **Write down what task 10's privilege check found**, once it runs: whether the Data API setting withholds privileges from `service_role` too, or only from `anon` and `authenticated`. This is no longer an open question to carry, it is a step inside task 10, but the answer belongs back in this spec because it decides how much else needs granting in feature 4 and after.

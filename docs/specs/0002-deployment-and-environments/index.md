...
- [ ] **What task 10's privilege check found**, on the hosted development project on 2026-08-22, in the dashboard query editor:
  ...
  - **Still open, and it is the half that decides feature 4.** `service_role` holding `select` on `app_settings` proves nothing about the Data API setting, because that grant is explicit in the migration. The isolating test is `public.scaffold_check`, which is granted to `authenticated` only and never to `service_role`. Locally `has_table_privilege('service_role', 'public.scaffold_check', 'select')` returns **false**, which says the explicit grant in the `app_settings` migration is load bearing and invariant 6 is real. Run the same query on the hosted development project and write the answer here.

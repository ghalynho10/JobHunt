...
- [x] In the Vercel project's environment variables, confirm the preview scope carries the **development** project's Supabase URL, publishable key and secret key, and that no production project value appears in any preview scoped variable. *Checked 2026-08-22 with `vercel env ls` and `vercel env pull` per environment: preview points at `serbucmdtvbspkbmxewl.supabase.co`, production at `fvaaebmjrrrjxxnaiyrb.supabase.co`, two distinct projects. `DEV_SESSION_ENABLED` is present on Preview and absent from Production, as designed.* → **AC-2**
- [ ] **Production's `SUPABASE_SECRET_KEY` is still unverified, and cannot be verified by inspection.** It is marked Sensitive in Vercel, which makes it write only: `vercel env pull` returns the literal string `[SENSITIVE]` rather than the value, so the two environments' keys cannot be compared. Preview's key is proved correct by behaviour, because the kill switch read the development database from a preview. Production's is proved only when production runs that same read after the merge. A key belonging to the wrong project fails as `401 Invalid API key`, which the kill switch reports as `database_unavailable`, which invariant 3 renders as switched on. So on the first production deploy, check `/health` reads **running** rather than assuming it. → **AC-2**, **AC-6**

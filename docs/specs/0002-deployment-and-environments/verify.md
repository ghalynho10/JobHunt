...
- [ ] Confirm the Sentry project shows trace sampling behaving as 1.0 in production and 0.1 in preview, and that neither file hardcodes it. → **AC-14**
  ...
  **Half proved on 2026-08-23.** A real production event's own Trace Details read `Client Sample Rate: 1`, confirmed directly rather than inferred from configuration. Neither Sentry config file hardcodes the value: both read `env.NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE`. Preview's `0.1` is still owed the same direct proof.

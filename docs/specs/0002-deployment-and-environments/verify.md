...
- [ ] **Production's `SUPABASE_SECRET_KEY` is unverified, cannot be verified by inspection, and cannot be exercised by the application either.** → **AC-2**, **AC-6**

  It is marked Sensitive in Vercel, which makes it write only: `vercel env pull` returns the literal string `[SENSITIVE]`, so the two environments' keys cannot be compared. Preview's key is proved correct by behaviour, because the kill switch read the development database from a preview.

  **Production's cannot be proved the same way, and an earlier version of this file wrongly said it could.** The suggestion was to check `/health` reads *running* after the merge. `/health` sits inside the protected layout, so it needs a session, and production has no sign in path until feature 7. Confirmed after the merge: production `/health` returns `307` to `/sign-in`, which returns `404`. Nothing deployed on production reads `app_settings` at all.

  So the key stays unexercised until feature 7 ships sign in, or feature 10 puts the kill switch in front of a gated call, whichever lands first. A key belonging to the wrong project fails as `401 Invalid API key`, which the kill switch reports as `database_unavailable`, which invariant 3 renders as switched on: it would look like a deliberate stop rather than a misconfiguration. **The cheap insurance is to re-copy the production secret key from its Supabase dashboard into Vercel's Production scope rather than waiting to find out.** Whichever feature first reads the switch on production inherits this check.

...
- [ ] Feature 14 confirms or corrects the "one scoring call per search" assumption behind `ai_scoring`'s account per week cap of 25; if scoring is per listing rather than per search, the multiplier belongs in a `usage_cap` update, not a migration. **The two numbers already in this spec contradict that assumption, and feature 14 should resolve the contradiction, not just the assumption**: `rationale.md`'s own token placeholder for `ai_scoring`, roughly 2000 input and 500 output tokens, is explicitly reasoned "from the shape of the task (one job listing plus one profile in, one structured score or verdict out)", the shape of a single-listing call. A cap of 25 per account per week sized on "one call per search" is only consistent with that placeholder if a search is scored as one listing, or if the real design batches every listing a search returns into one call scored against that same ~2000/500 token budget regardless of how many listings are inside it, an estimate that does not scale with listing count and was not derived assuming it would. Feature 11 (job search & results list), which would set how many listings a search returns, is not designed yet, so this cannot be resolved from either spec alone; it is `/architect fit scoring`'s decision to make.

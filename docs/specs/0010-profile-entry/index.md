...
- **AC-5**: ~~Skills are entered as one field, one skill per line, and saved as a whole.~~ · **SUPERSEDED 2026-09-23 by spec [0023](../0023-chip-input-for-skills-titles-locations/index.md) AC-1, for the JavaScript path only.** Not edited into agreement and not deleted, for the same reason spec 0006 AC-7 was kept struck through: it was a deliberate, well argued criterion for as long as a textarea was the only control this product had, and a later reader should see why it stopped applying rather than find a spec that quietly disagrees with the product. With JavaScript available, skills are now typed one at a time and committed to a removable chip on Enter (spec 0023's chip control), still submitted and saved as one whole list. **The struck sentence remains exactly true, unchanged, whenever spec 0023's chip control has not mounted, including with JavaScript disabled entirely**: spec 0023 AC-4 keeps this same textarea, one skill per line, as the normative fallback, and the sentence describes it correctly today and after spec 0023 alike. What survives untouched everywhere, restated by spec 0023 AC-3: saving compares the submitted list against the caller's current `profile_skill` rows, case insensitively, matching the unique index that is the authority on skill identity, and deletes only the names that were removed and inserts only the names that are new, never a blanket delete followed by a blanket insert. A submission that changes only a name's capitalisation (`react` for stored `React`) is a no-op; the stored casing stands.
- **AC-6**: The skills parser trims each line, drops empty lines, removes duplicates ignoring case, and enforces the 100 character per name limit before writing, matching the unique index on `(profile_id, lower(name))`. A name that still collides after that cleanup is a visible error, not a silent drop. **Unchanged by spec 0023.** Nothing here is struck: spec 0023 AC-15 keeps this parser (`newlineList()`) and its behaviour exactly as described, whether the newline separated string it receives was typed directly into the textarea above or assembled by spec 0023's chip control from the values committed there before submission. A chip commit applies the same trim and the same empty value drop before it ever becomes a chip (spec 0023 AC-6), which is the chip control's own echo of this rule, not a replacement for it.
...
- **AC-9**: Search preferences (`desired_titles`, `desired_locations`, `remote_preference`, `minimum_pay`, `minimum_pay_currency`) are edited together as one section. ~~`desired_titles` and `desired_locations` are each entered as one field, one value per line, never comma separated (a location value can itself contain a comma);~~ · **SUPERSEDED 2026-09-23 by spec [0023](../0023-chip-input-for-skills-titles-locations/index.md) AC-1, AC-2, for the JavaScript path only.** With JavaScript available, `desired_titles` and `desired_locations` are now entered as removable chips, one value committed per Enter press; a value can still contain a comma ("Chicago, IL" stays one chip), and commit never splits on one. **The struck sentence remains exactly true, unchanged, whenever spec 0023's chip control has not mounted, including with JavaScript disabled entirely**: spec 0023 AC-4 keeps this same textarea, one value per line, as the normative fallback. Each line is trimmed, empty lines are dropped, values are deduplicated ignoring case the same way skills are, each value is at most 100 characters, and each list holds at most 50 values; **unchanged by spec 0023 on both paths**, restated by spec 0023 AC-6 and its Key invariants, since a chip commit applies the same trim, dedupe, and limit checks before the value is ever assembled into the string this parser receives. `remote_preference` is a closed choice among `on_site`, `hybrid`, `remote`, `no_preference`. `minimum_pay` and `minimum_pay_currency` are both present or both absent; an empty `minimum_pay` field is treated as absent before that pairing check runs, never as zero. When present, `minimum_pay` is at least 0, at most 9,999,999,999.99, and at most two decimal places, rejected rather than rounded; `minimum_pay_currency` is a free text field, trimmed and uppercased before checking, then exactly three uppercase letters, with no fixed currency list (the schema has none, and one here would be a second source of truth).

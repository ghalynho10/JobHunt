...
- **AC-4**: At least three pairs share one archetype and near identical posting skill and experience requirements, differing only in whether the posting conflicts with that archetype's own `preferences` (which the prompt does send, `rubric.ts:437-463`). Between them they cover all four preference dimensions spec 0015's Follow up names, **title, location, remote, and pay**: the stated title against `desired_titles`, the stated location against `desired_locations`, the on site or remote language against `remote_preference`, and the pay figure against `minimum_pay`. Every conflict is written into `title`, `location` or `descriptionSnippet` (three of the four fields `buildScoringPrompt` sends, alongside `companyName`; salary fields are never sent, so the pay conflict has to live in `descriptionSnippet`'s text, never in `salaryMin`/`salaryMax`). Every one of these pairs carries the same `expectedBand`. A harness run scoring them differently is evidence that a stated preference moved the band, which spec 0015's own instructions forbid.
  ...
  **Grouped by class of preference, not one pair per dimension.** This is a decision, not an accident of how the set grew, and it is what the third pair's existence turns on. The four dimensions fall into two classes, and a pair isolates a class:

  - **Circumstance preferences** (location, remote, pay): what surrounds the job. All three are varied together in one pair, because they share a single failure mode (the model reading the candidate's stated circumstances as a fit signal) and none of them has any counterpart among the fields the prompt actually scores. One pair per dimension here would triple the vendor calls this class costs on every harness run to buy a diagnosis that bisecting afterwards gives for free.
  - **The title preference** (title): what the job is *called*. Isolated alone, because it is the only one of the four whose value has a direct counterpart among the four listing fields `buildScoringPrompt` sends. The prompt states `Title: <the posting's title>` in the posting section and `- Desired titles: <the candidate's list>` in the preferences section, so a model has both halves of a comparison in front of it and nothing but the written instruction telling it not to make one. That is a materially different risk from a pay figure, which has no scored counterpart at all, and it is why grouping title in with the other three would hide exactly the leak most likely to happen.

  A fifth preference dimension, if one is ever added to the profile feature, joins the class it belongs to rather than getting an automatic pair of its own: ask whether its value has a counterpart among the scored listing fields.

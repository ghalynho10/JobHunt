...
- **AC-5**: Vitest runs unit and integration tests, and integration tests run against the local Supabase stack with the real policies applied, not a mock. Playwright is scaffolded but carries no real end to end test yet.
...
**Chosen option**: Option 2 in rationale.md (Vitest integration against the real local stack, Playwright scaffolded but unused for now). The sub decisions below follow the options marked chosen in rationale.md.

One sentence: Test foundation uses Vitest (unit and integration) with Playwright scaffolded, a development only session mint through the admin magiclink exchange, a bespoke recorder that replays real recorded responses from files, a re-minted fixture pool with valid ids, fresh on demand users for write tests, an in memory Sentry transport the test can read rather than a disabled one, and a CI job that runs integration against the local Supabase stack.

Sub decisions, all binding for the build:
- **Runners**: Vitest for unit and integration. Playwright is scaffolded (config, no real test) and gets its first real end to end test in a later feature.
...
- **Session mint**: a development only helper under `src/lib/testing` that uses the secret client (binding rule 1, caller 1) to generate an admin magiclink for a fixture user and exchanges it with `verifyOtp` through the request scoped client into an in memory cookie jar. Guarded by `DEV_SESSION_ENABLED` (default false, fails closed). Programmatic only, no HTTP route. It does not depend on the development password sign in that feature 7 deletes.
- **Record and replay**: a bespoke recorder at the fetch boundary. Record mode (explicit opt in, warns) saves a real response to `test/fixtures/<service>/`. Replay mode returns recorded responses and never touches the network. A missing fixture fails loudly, naming the file and the record command. Proven now on a stand in recorded response; Adzuna and the model router record their own later.
...
**API surface**:
The surfaces are programmatic helpers in test code, not HTTP endpoints. The session mint cannot be a route under `src/app` (binding rule 1 forbids importing the secret client from there), and it must not live in `src/features/dev-session` (feature 7 deletes that folder). Both helpers live under `src/lib/testing`.
...
**Security model**:
- The session mint is a secret key caller (binding rule 1, slot 1), hard blocked outside development, and lives under `src/lib`, never under `src/app`. Note what the lint rule actually covers: the `@typescript-eslint/no-restricted-imports` override in `eslint.config.mjs` blocks `src/app/**` from importing `**/supabase/secret`, and it does not block `src/app/**` from importing the mint, which imports that module transitively. The mint therefore opens a path around the rule that the rule cannot currently see. Closing it is a follow-up below, not an assumption this design may rest on.
...
- Record and replay: record a real response from the stand in endpoint once, commit it, replay it, verifies **AC-2**, **AC-8**
...
Tracer Bullet: stand up the thinnest real thread first (a real session, a real read, real isolation), then thicken with record and replay, on demand mints, commands, CI, and Playwright.

1. Scaffold Vitest as two projects: a unit project (jsdom only where a test renders a component) and an integration project (`node`, because `@t3-oss/env-nextjs` will not hand a server variable to code it believes runs on the client). Alias `server-only` to an empty module, resolve `@/` with `vite-tsconfig-paths`, load `.env.test`, and install the in memory Sentry transport in the setup file. Add `pnpm test` as `vitest run`, satisfies **AC-5**, **AC-10**
...
4. Build the development only session mint under `src/lib/testing`, guarded by `DEV_SESSION_ENABLED`, using the admin magiclink exchange into an in memory cookie jar passed to `createClient()`. Verify the exact Supabase auth API surface against current docs at build time (per the supabase skill), satisfies **AC-3**
...
6. Build the bespoke record and replay recorder with an explicit record mode, replay from `test/fixtures/<service>/`, redaction at write time, and record the stand in response. Add `test/fixtures/` to `.prettierignore` in the same step, satisfies **AC-2**, **AC-8**, **AC-9**, **AC-13**
...
10. Scaffold Playwright with a placeholder config and no real test yet, unless the follow-up below settles on dropping it, satisfies **AC-5**
...
**Neutral**:
- Playwright is scaffolded but inert until a feature needs a browser.
...
- [ ] Playwright's first real end to end test lands with the first feature that needs a browser (feature 9 onward).
...
- [ ] Extend the binding rule 1 lint rule to cover the mint, or keep the helpers out of `src/` entirely. The `@typescript-eslint/no-restricted-imports` override in `eslint.config.mjs` blocks `src/app/**` from importing `**/supabase/secret`, and nothing blocks `src/app/**` from importing the mint under `src/lib/testing`, which imports that module transitively. Either add the testing path to the restricted patterns, or move the helpers to a top level tree the application's module graph never reaches. Until one of the two lands, the enforcement binding rule 1 claims is one import short.
...
- [ ] Name the stand in endpoint the first recording is captured from, before build step 6 improvises one. It has to be real, free, stable and unauthenticated.
- [ ] Decide where test files live: colocated beside the code under `src/features/<feature>/`, following the folder by feature rule, or a top level `test/` tree. Every feature after this one inherits the answer. `tsconfig.json` includes `**/*.ts`, so either is type checked without a change.
- [ ] Reconsider whether Playwright is scaffolded here at all. Build step 10 adds a dependency, a config file and an empty suite for no proof today, and the runner decision is recorded in this spec whether or not the config exists. Dropping it costs nothing, and feature 9 adds it alongside its first real test.

...
- [ ] Same source view → `<title>` is `JobHunt`, the description is the one in the spec's `## Copy`, and `twitter:card` is `summary_large_image` → AC-10 · **partly done 2026-08-28:** `<title>` and the description were confirmed against the spec's `## Copy`. `twitter:card` was not reported, so this stays open on that one attribute
...
- [ ] Tab from the top of the page → the logo link, then each nav anchor, then Sign in, each showing a visible teal focus ring that appears instantly rather than fading in → AC-14 · **partly done 2026-08-28:** the header logo's focus ring was confirmed visible. The rest of the tab order (each nav anchor, then Sign in) was not reported, so this stays open
...
**Manual pass, 2026-08-28.** Seven of thirteen manual steps confirmed against the signed in preview. Six remain, and none of them is a failure: three are blocked by deployment protection or by the branch not being merged, one waits on build step 9 to create the sign in band, and two were partly done (`twitter:card` and the full tab order past the header logo). The blocked and partial reasons are recorded on each line rather than summarised away, so a later reader can tell a step that was skipped for a reason from one that was ducked.

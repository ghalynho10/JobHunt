...
- [ ] Each rule's query filters by failure kind (the span status message), not by span status alone: confirm a deliberately triggered `session_missing` does **not** move `usage_gate.check`'s ratio, and a deliberate cap refusal does not either. Verifies **AC-5**, **AC-10**.

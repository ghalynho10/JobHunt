...
- [x] Confirm each recipient entry on the page came from `recipients.ts` by changing one `receives` string and reloading → the page text changes, proving the prose is rendered and not restated → AC-3, AC-6

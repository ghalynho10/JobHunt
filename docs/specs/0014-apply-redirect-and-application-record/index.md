...
- **AC-20**: The apply action builds its Supabase client with a **read only cookie adapter**, so nothing inside it can write a session cookie. A cookie write inside a Server Action puts a re-render of the current route into the action's response, which on `/search` means a second Adzuna call, so this is what makes AC-10 hold for an expired session rather than only for a fresh one. The refusal to write is deliberate and documented at the call site: `src/proxy.ts` refreshes the session on every request, which is the same thing the existing adapter's own catch block already relies on.
- **AC-21**: An apply attempted against a stale build fails visibly, carrying `COPY-7`. The encryption key protecting the bound listing is regenerated on every build, so any results page left open across a deploy has an apply control the framework will refuse before this feature's code runs. The failure is caught and shown rather than left as a silent no operation.
...
2. The action mutating a cookie. This is the one nobody writes on purpose: the shared cookie adapter writes cookies, and a session refresh during the action's own caller check would trigger one. Prevented by AC-20's read only adapter.
...
| What | From | To |
|---|---|---|
| `AdzunaAttribution`, `JobsworthAttribution` | `src/features/search/adzuna-attribution.tsx` | `src/components/ui/` |
| `salaryText`, `relativePostedAt` | `src/features/search/result-card.tsx` (the first is currently unexported) | `src/lib/` |
| `ADZUNA_SOURCE`, `ADZUNA_ATTRIBUTION_URL`, `ADZUNA_JOBSWORTH_URL` | already exported from `src/features/search/adzuna.ts` | `src/lib/`, so neither feature imports from the other |

**API surface**:

| Surface | Kind | Key inputs | Key outputs | Auth | Key errors |
|---|---|---|---|---|---|
| `recordApplication` | Server Action, reached through an inline per card closure | the `Listing`, captured by the closure and re-parsed by `listingSnapshotSchema` on arrival | `ActionState`: `status: "applied"` on success, `"failed"` with a message otherwise. Never `undefined`, which would be indistinguishable from the initial state | own caller verified inside the action, using the read only cookie adapter | duplicate (`validation_failed`), no profile row (`record_not_found`), bad shape or unparseable date (`validation_failed`), stale build (`validation_failed`), database down (`database_unavailable`) |
...
| record | the stale build message | `COPY-7`, from the caught framework level refusal (AC-21) |
...
- Expired session apply: an apply on a session whose access token has expired spends no Adzuna call, which is the case the read only adapter exists for and the one a single fresh session measurement misses, verifies **AC-10**, **AC-20**.
...
- A deploy invalidates the apply control on any results page left open, because the closure key is regenerated per build. AC-21 makes that visible rather than silent, and it cannot make it not happen.
...
- [ ] **Settle the one claim this spec inferred rather than verified.** AC-20's read only adapter is built on the reasoning that `getClaims()` can refresh an expired access token and persist new cookies through `setAll`. The cookie write path is verified (`src/lib/supabase/server.ts:56-60`) and the re-render trigger is verified (`server-actions.md:47`); the refresh on expiry in the middle is not. The fix is correct either way and costs nothing if the refresh never happens, but the reasoning should be confirmed rather than inherited, and the answer written back here.
...
- [ ] **Correct spec 0003's `job_description` description in both places, not one.** [index.md:118](../0003-data-model/index.md) calls it "the listing's full description text. The only copy that survives the posting being taken down", and [index.md:262](../0003-data-model/index.md) says it "stores a full listing body per application". Both are wrong: Adzuna's search response carries only a snippet. Spec 0013's own Follow-up item names only line 118, so that item is itself incomplete and should be updated when it is closed. A plain grep for "full text" or "full description" finds only the first; line 262 says "full listing body" and is missed.
- [ ] **Amend spec 0013's Decision at [index.md:42](../0013-job-search-and-results-list/index.md) with a visible note** that "never a Server Action or a client side fetch" and "no client JavaScript ships for search itself" no longer hold in full once this feature ships. Follow the shape of that spec's own AC-10 correction: a note in the spec that a later reader cannot miss, rather than a quiet narrowing recorded only in this spec's Consequences.
- [ ] **Update spec 0013's `Listing` table at [index.md:63-65](../0013-job-search-and-results-list/index.md)** so `sourceJobId`, `title` and `companyName` record the non empty guarantee AC-13 adds. The table currently documents all three as plain strings, which is what let the gap sit undetected between a shipped schema and a shipped table. Its `postedAt` row also claims ISO datetime while the code accepts any string, which AC-2 now enforces at this feature's boundary and which that table should say.
- [ ] **Close spec 0003's Follow-up items at lines 274 and 275.** Line 274 asks for a `Done when` criterion covering the missing profile case, which AC-5 now supplies. Line 275 asks for the predicted salary decision, which AC-6 now settles. Both were written at design time and neither had been acted on.

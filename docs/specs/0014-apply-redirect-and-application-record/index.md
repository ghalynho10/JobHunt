...
- **AC-21**: An apply attempted against a stale build fails visibly, carrying `COPY-7`. The encryption key protecting the bound listing is regenerated on every build, so any results page left open across a deploy has an apply control the framework refuses. **The refusal happens before any of this feature's server code runs, so it is caught in the client control around the call, never in `recordApplication`** (corrected 2026-09-05, see `## Consequences`). **A refused dispatch was observed re-rendering `/search`, which re-runs the search and spends one of the 25 weekly Adzuna calls.** That observation came from a request without JavaScript (a hand built `POST`, the only shape that can be driven headlessly), and whether a hydrated control's dispatch does the same or instead rejects the promise the client catch is waiting for is **not yet known**: see `## Follow-up`. Either way the message is not politeness about a dead button, and if the spend does carry over to the hydrated path then the reader is being charged for a failure they did not cause.
...
- **A deploy invalidates the apply control on any results page left open, and it costs the reader a search.** The closure key is regenerated per build, so the framework refuses the dispatch. Observed for real on 2026-09-05 against a production build: the server logs `Failed to find Server Action. This request might be from an older or newer deployment.` and then **falls back to re-rendering `/search`, which re-runs the Adzuna search and spends one of the 25 weekly calls**. **Read the limit of that observation before relying on it:** it was driven by a hand built `POST`, the no JavaScript request shape, because a hydrated dispatch cannot be driven headlessly at all. The hydrated path may instead reject the promise without any re-render. So the spend is confirmed for one request shape and unknown for the one real readers send. If it does carry over, a stale apply is worse than a dead button, because it silently bills the reader for a failure they did not cause; AC-21 makes the failure visible either way and cannot make it not happen.
- **The stale build refusal cannot be handled where a failure normally is.** Next rejects the dispatch before `recordApplication` executes, so this feature's failure table, its span, and its whole error model never see it. The catch sits in `ApplyControl` around the call instead, which is the only place left, and it is a plain `try`/`catch` around everything rather than a match on the framework's wording, because that wording is not ours to depend on. One consequence worth naming: this is the only failure in the feature that never reaches Sentry through `failure()`, because no server code runs to report it.

...
- [ ] **Production build.** Note the `job_search` weekly counter, run one search, mark two results applied, re-read the counter → it moved by exactly one, not three → AC-10
- [ ] **Production build.** With the network tab open, mark a result applied → no request to `api.adzuna.com` is made → AC-10
...
- [ ] Apply, then run `pnpm build` again to rotate the encryption key, then press the apply control on the page still open from before → a visible message (`COPY-7`), not a silent no operation and not an unhandled framework error → AC-21
...
- [ ] View source on a rendered results page (the **served HTML**, not the JavaScript bundle) and search it for a listing's `job_url` and `source_job_id` as they appear in the card → they are not present in plain text anywhere the action's payload travels. **Read this step's history before running it**: the first version grepped the built client bundle, where the closure never lands at all, so it would have passed identically against unencrypted `.bind()` arguments. That is the verify step shape `docs/reflexes.md` records as having escaped twice → AC-2

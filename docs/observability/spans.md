...
| `search.run` | `http.client` | `src/features/search/adzuna.ts` | Not yet. It wraps the one outbound Adzuna call `withUsageGate()` guards, so its failure ratio is `external_service_failed` and `response_malformed` against every attempt, never a gate refusal (spec 0013, AC-3, AC-5, AC-10), which never reaches this span at all since `withUsageGate()` short circuits before calling in. |

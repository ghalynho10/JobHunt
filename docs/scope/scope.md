...
- [ ] Build it: `/develop auth & per user isolation`
  ...
  - [ ] The entry page and the record: the two provider labels become real submits with `COPY-1` and both status chips deleted, spec 0006's "no dead controls" test split so its apply half survives, and the session policy matrix read off both dashboards, satisfies AC-8, AC-15, AC-16, AC-19 · **AC-16 and AC-19 done 2026-08-30, AC-8 and AC-15 on real accounts are not**, which is why this stays open. Both controls post to real Server Actions in the hero and the band, `COPY-1` and both chips are gone, and the band's own "is coming soon" clause went with them on the same grounds even though AC-16 does not name it. The page level test is split: the apply control keeps its half unchanged and the provider half asserts all four forms post to an action. `/` still prerenders as static, so spec 0006 **AC-4** holds. AC-8 was proved against a stand in provider (one user, two identities, same id) but not yet with the two real accounts

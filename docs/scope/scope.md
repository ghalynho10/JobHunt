...
- [ ] Build it: `/develop data model`
  ...
  - [ ] Proved against the real project: isolation both ways, the `with check` refusals, every constraint, the privilege check, and the preview confirmed as all three seeded users (AC-2 to AC-11, AC-14) · **half done**: every one of these passed against the local database, and all three seeded users were driven through the real page (own profile, the other's profile, and the named `record_not_found` for the user with none). What is left is the same sweep against the hosted development project and a real preview URL, which needs the pull request pushed

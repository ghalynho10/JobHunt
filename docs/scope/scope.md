...
- [ ] Build it: `/develop entry page & link metadata`
  ...
  - [x] Compose and pass: replace the scaffold placeholder, then run the keyboard, focus, responsive and reduced motion pass at 320 and 1440 pixels and prove no client JavaScript ships, satisfies AC-1, AC-2, AC-4, AC-14 · landed 2026-08-28, run against the production build in a real browser. Measured rather than eyeballed: the three rhythm tiers distinct at both widths (160 / 64 / 96 pixels at 1440, 128 / 48 / 80 at 320), zero horizontal overflow at 320 with no element crossing the viewport edge, all five controls keyboard reachable in order with a 2px teal `:focus-visible` ring, the header's Sign in jumping to the band at both widths, and no `"use client"` anywhere in the page's tree. **One defect found, and it is not this feature's file:** under `prefers-reduced-motion: reduce` the match cells still appear one at a time, because the global override in `globals.css` zeroes `animation-duration` but leaves `animation-delay`, so the last cell is still invisible 350ms after load. Spec 0005 owns that line

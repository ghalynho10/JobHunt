...
- [ ] Build it: `/develop auth & per user isolation`
  ...
  - [ ] The thin thread, Google only, proved on a preview: both provider stanzas in `config.toml` with the local redirect URLs corrected, `src/features/auth/` with `signInWithGoogle()` and the `/auth/callback` route handler, `/sign-in` rebuilt as a real page with the five member error enum, satisfies AC-1, AC-2, AC-3, AC-4, AC-5, AC-6, AC-7, AC-12, AC-18 · **code done 2026-08-30, the preview leg is not**, which is why this stays open. Proved locally instead: the action returns a GoTrue authorize URL carrying `redirect_to=http://localhost:3000/auth/callback` and writes a host only PKCE verifier cookie with no `Domain` attribute, which is AC-4's premise observed rather than assumed, and all four callback paths 303 to their own code. Two things the spec left open were settled empirically against CLI 2.115.0: the Supabase CLI reads `env()` values from `supabase/.env` **and** from the process environment, which is what lets CI pass placeholders. Ticking this needs a deploy and a real Google sign in on the branch URL, which also closes spec 0002's blocked `currentOrigin()` verify step

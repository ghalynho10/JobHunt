...
_spec [0007](../specs/0007-auth-and-per-user-isolation/index.md)_
...
- [ ] Build it: `/develop auth & per user isolation`
  - [ ] Prerequisites, confirmed rather than assumed: both OAuth applications, GoTrue's callback registered three times per provider, the two redirect allowlists (exact on production, branch shaped wildcard on development), the linking default and the GitHub `user:email` behaviour, satisfies AC-20
  - [ ] The thin thread, Google only, proved on a preview: both provider stanzas in `config.toml` with the local redirect URLs corrected, `src/features/auth/` with `signInWithGoogle()` and the `/auth/callback` route handler, `/sign-in` rebuilt as a real page with the five member error enum, satisfies AC-1, AC-2, AC-3, AC-4, AC-5, AC-6, AC-7, AC-12, AC-18
  - [ ] Thicken: GitHub with the `user:email` scope, sign out rebuilt in `src/features/auth/` with its span and failure path, then the deletions (`src/features/dev-session/`, `src/lib/supabase/browser.ts`) and the span registry, satisfies AC-1, AC-11, AC-12, AC-13, AC-17
  - [ ] The account rule: the `public.before_user_created_hook` migration (`security definer`, empty `search_path`, explicit grants) rejecting on its own internal error too, enabled in both hosted dashboards, P10 answered against the local stack and `COPY-2` wired, plus the integration suite, satisfies AC-9, AC-10, AC-14, AC-15, AC-19
  - [ ] The entry page and the record: the two provider labels become real submits with `COPY-1` and both status chips deleted, spec 0006's "no dead controls" test split so its apply half survives, and the session policy matrix read off both dashboards, satisfies AC-8, AC-15, AC-16, AC-19

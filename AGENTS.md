...
pnpm eval             # PAID: scores the ground truth set for real, 80 vendor calls
```

Tests run on Vitest, as four projects: `unit` and `integration` (spec 0004), `integration-serial` (spec 0011) and `eval` (spec 0017). `pnpm test` is the unit suite and needs nothing running; `pnpm test:integration` drives the real local Supabase stack with the real policies and fails with a named message when the stack is down, and it runs the serial project too. **`pnpm eval` spends real money**, five vendor calls per ground truth pair and 80 for the full set, so it is reachable only by naming `--project eval`, which that script wraps, and it is in neither of the other two commands. The ceiling that actually binds it is the shared `ai_scoring` global day cap, not the account weekly cap, because the harness mints a fresh fixture user every run. Unit tests sit beside the code they prove (`src/**/*.test.ts`); the session mint, the recorder, the fixtures and the direct database connection live in `test/`, outside `src/`, so no application module can import a test helper. The direct connection (`test/helpers/database.ts`, feature 7) is the widest privilege in the repository: it exists because spec 0007's refusal hook is deliberately unreachable from the Data API, it is guarded by `TEST_DIRECT_DB_ENABLED`, and it refuses any host that is not local. No end to end runner is installed: Playwright is the recorded choice and arrives with the first feature that needs a browser.
...
- **A key is not a policy.** Moving to publishable and secret keys protects nothing on a table without row level security, so every new table ships with RLS enabled and forced in its own migration. All nine tables have it today (spec 0003 AC-2 for the six holding user data, specs 0002 and 0011 for the other three).
- **Authorisation is never decided in the proxy.** [src/proxy.ts](src/proxy.ts) only refreshes the session cookie, and it is `proxy.ts` with a `proxy` export, never `middleware.ts`, which Next.js 16 deprecated. The protected layout checks the session and every Server Action checks its own caller independently. Row level security in Postgres is the real guarantee. Route handlers under `src/app/api/` may not read or write user data. **One exception to the refresh, and it is about where the refreshed cookie lands rather than whether it happens**: on a Server Action request the proxy hands it to that request's own code and withholds it from the response, because a cookie mutated during an action makes Next re-render the current route, and on `/search` that costs an Adzuna call (spec [0001](docs/specs/0001-stack-and-architecture/index.md) binding rule 6, spec [0008](docs/specs/0008-app-shell-and-navigation/index.md) AC-10b). It keys on the `next-action` header and never on the route, which is what keeps this rule's mechanical guard intact; do not rewrite it to test a path.
- **Verify the caller with `getClaims()`, never `getSession()`.** `getSession()` reads the cookie without verifying it against the auth server, so a check built on it passes without proving anything. Spec 0011 AC-13 states this for the usage gate; it holds for every server side identity check.
...
- **Folder by feature.** A feature's code lives in `src/features/<feature>/`. Routes live only in `src/app`. Anything two features share moves to one of three homes: `src/lib` for logic, `src/components/ui` for a design system primitive, and `src/components` for a shared component that is not one. The third exists because feature 12 needed a vendor licence attribution in two features, and [src/components/ui/AGENTS.md](src/components/ui/AGENTS.md) defines that directory as spec 0005's design system, "the only sanctioned way to render these patterns", which an attribution is not (spec [0014](docs/specs/0014-apply-redirect-and-application-record/index.md)).
...
- **Before push**: [.husky/pre-push](.husky/pre-push) fetches `origin main` and says how far behind this checkout is, naming any rule file the incoming commits touch (`docs/reflexes.md`, `AGENTS.md`, `CLAUDE.md`, `.claude/`). It **warns and never blocks**, deliberately: a stacked branch or one based on an older point is behind by design, and a hook that blocked those would be bypassed with `--no-verify` until it protected nothing. The failure it exists for is not knowing.

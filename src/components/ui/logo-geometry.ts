/**
 * The JobHunt logo's raw geometry, lifted from `docs/design/logo/lockup.svg`.
 *
 * WHY THIS IS DATA IN ITS OWN MODULE AND NOT MARKUP INSIDE `logo.tsx`. Two very
 * different renderers draw this logo: React, for the page, and Satori, for the
 * social preview image, which never sees a stylesheet and cannot render a React
 * component. Without a shared source they would each hold their own copy of a
 * 2655 character path, and the two would drift the first time the mark changed.
 *
 * `logo.test.ts` reads both assets back off disk and fails if either constant
 * stops matching, so the copy is guarded rather than merely intended.
 */

/** The bracket mark's five rectangles, on the 32 unit master grid. */
export const MARK_RECTS = [
  { x: 2, y: 2, width: 11, height: 3 },
  { x: 2, y: 2, width: 3, height: 11 },
  { x: 12, y: 12, width: 8, height: 8 },
  { x: 19, y: 27, width: 11, height: 3 },
  { x: 27, y: 19, width: 3, height: 11 },
] as const;

/**
 * "JobHunt" as outlined vector paths, never text.
 *
 * This is what lets the preview image draw the wordmark without shipping font
 * bytes to do it: the shapes are already outlines, so they render identically
 * whether or not Space Grotesk loaded.
 */
...
/** Where the wordmark sits once the mark grid has been scaled up beside it. */
export const WORDMARK_OFFSET_X = 87.5;

/** The 32 unit mark grid, scaled to the 70 unit wordmark height. */
export const MARK_SCALE = 2.1875;

/** The lockup's own coordinate space: mark plus wordmark, side by side. */
export const LOCKUP_VIEWBOX = "0 0 484.26 71.4";
